...
q2 = prompt("Você sabe utilizar um canhão de prótons?").toLocaleLowerCase();
if (q2 == "sim") {
  respostas.push("sim");
}
q3 = prompt(
  `Você tem conhecimento para  consertar os motores de uma nave?`
).toLocaleLowerCase();
if (q3 == "sim") {
  respostas.push("sim");
}
q4 = prompt(
  `Possui conhecimento sobre viagens interdimensionais?`
).toLocaleLowerCase();
if (q4 == "sim") {
  respostas.push("sim");
}
q5 = prompt(
  `Você está pronto para se sacrificar em prol da humanidade?`
).toLocaleLowerCase();
if (q5 == "sim") {
  respostas.push("sim");
}
switch (respostas.length) {
  case 0:
    console.log(
      `Você falhou miseravelmente ${personagem}, agora viva com a destruição do mundo em suas mãos.`
    );
    break;
  case 1:
    console.log(
      `Você é um covarde ${personagem}, fugir não adianta nada, iremos morrer assim.`
    );
    break;
  case 2:
    console.log(
  ...
  case 3:
    console.log(
      `Após diversas tentativas você quase conseguiu ${personagem}, mas não foi o suciente, estamos condenados a morte.`
    );
    break;
  case 4:
  ...
  case 5:
    console.log(
      `Parabéns ${personagem}, você destruiu o meteoro facilmente, talvez você deva se tornar um viajante inter-dimensional.`
    );
    break;
}
